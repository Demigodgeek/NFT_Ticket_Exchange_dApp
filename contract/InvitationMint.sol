...
contract InvitationMint is ERC1155, Ownable {
    ...
    mapping(uint256 => address) private originalMinters;
    ...
    function mintInvitation(uint256 invitationId) external payable {
        ...
        originalMinters[invitationId] = msg.sender;
    ...
    function getOriginalMinter(uint256 invitationId) external view returns (address) {
        return originalMinters[invitationId];
    }
    

    function generateReceipt(uint256 invitationId) external {
        require(hasMintedId[invitationId], "Invalid invitationId");
        address originalMinter = originalMinters[invitationId];
        address currentOwner = invitationOwners[invitationId];
        require(msg.sender == originalMinter || msg.sender == currentOwner, "You didn't mint that invitation");
