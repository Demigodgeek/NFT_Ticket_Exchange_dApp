...
contract_address = "0x745bc5653E44BA8260FBe2fE73f2c27B6631A9B0"
...
if user_type == "Administrator":
    ...
    operation = st.sidebar.selectbox("Choose an operation", ("Contract Balance", " Withdraw Contract Balance"))

    if operation == "Contract Balance":
    ...
    if operation == "Purchase Invitation":
        ...
        st.markdown("<ul>"
                    ...
                    "If you want to gift the invitation to someone else, please generate a receipt before gifting. You would not be allowed to generate the receipt since you are no longer the owner."
                    "</li>"
        ...
        if st.button("Generate Receipt"):
            if Web3.isAddress(receipt_owner):
                ...
                if receipt_result is not None:
                    ...
                    st.warning("You didn't mint that invitation, or you are no longer the invitation owner. Receipt generation is not allowed.")
            else:
                st.warning("Invalid sender address.")
        ...
        if st.button("Gift Invitation"):
            if Web3.isAddress(gift_sender) and Web3.isAddress(recipient):
                ...
                if owner_address.lower() == gift_sender.lower() and gift_sender.lower() != recipient.lower():
                    ...
                    st.warning("Recipient address should be different from sender address.")
            else:
